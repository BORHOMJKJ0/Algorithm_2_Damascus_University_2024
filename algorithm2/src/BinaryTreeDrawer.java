...
public class BinaryTreeDrawer extends JPanel {
    ...
    private void calculateNodePositions(binaryTree node, int x, int y, int xOffset) {
        if (node == null) {
            return;
        }
    ...
    private void drawTree(Graphics2D g, binaryTree node) {
        if (node == null) {
            return;
        }
